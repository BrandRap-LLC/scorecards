...
interface PaidGridProps {
  clinic?: string;
  startDate?: string;
  endDate?: string;
}

export default function PaidGrid({ clinic, startDate, endDate }: PaidGridProps) {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(`/api/paid-ads?${params}`);
  ...
  if (data.length === 0) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-gray-500">No paid ads data available</div>
      </div>
    );
  }
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {data.map((record, index) => (
            <tr key={index} className="hover:bg-gray-50">
              ...
              <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 text-right">
                {record.spend !== null ? formatCurrency(record.spend) : '-'}
              </td>
              <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 text-right">
