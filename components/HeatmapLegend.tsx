  ...
  return (
    <div className={`bg-white rounded-lg shadow-sm p-3 ${className}`}>
      ...
      <div className="space-y-2">
        ...
        <div>
          <p className="text-xs text-gray-600 mb-1">Standard Metrics (higher is better):</p>
          <div className="flex items-center gap-1">
            {HEATMAP_LEGEND.normal.map((item, index) => (
              <div key={index} className="flex items-center gap-1">
                <div className={`w-4 h-4 ${item.color} rounded border border-gray-200`}></div>
                <span className="text-xs text-gray-600">{item.label}</span>
              </div>
            ))}
            <div className="flex items-center gap-1 ml-2">
              <div className={`w-4 h-4 ${HEATMAP_LEGEND.null.color} rounded border border-gray-200`}></div>
        ...
        {showInverted && (
          <div>
            <p className="text-xs text-gray-600 mb-1">Cost Metrics (lower is better):</p>
            <div className="flex items-center gap-1">
