...
async function syncExecutiveMonthly() {
  ...
  // Using hardcoded values that work in test script
  const mssqlConfig = {
  ...
  try {
    ...
    // Get all data from MSSQL - first check what columns exist
    const columnCheck = await mssqlPool.request()
      .query(`
        SELECT TOP 1 * FROM executive_report_new_month
      `);
    
    console.log('📋 Available columns in MSSQL:');
    console.log(Object.keys(columnCheck.recordset[0]).join(', '));
    console.log('\n');
    ...
    if (deleteError) {
      console.log('⚠️  No existing data to clear or table might not exist');
    } else {
    ...
    for (let i = 0; i < result.recordset.length; i += batchSize) {
      ...
      // Map to ALL fields that exist in the Supabase table
      ...
      if (insertError) {
        ...
        // Log sample data that's failing
        console.log('\nSample of failing data:');
