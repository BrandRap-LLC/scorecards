...
function sanitizeNumeric(value, fieldName = '') {
  ...
  // Cap extremely large values - DECIMAL(10,2) can hold max 99,999,999.99
  const MAX_SAFE_VALUE = 99999999; // Safe limit for DECIMAL(10,2)
  ...
  try {
    ...
    for (let i = 0; i < result.recordset.length; i += batchSize) {
      ...
      const transformedBatch = batch.map(row => ({
        ...
        impressions: sanitizeNumeric(row.impressions, 'impressions'),
        clicks: sanitizeNumeric(row.visits, 'clicks'), // Using visits as clicks
        spend: sanitizeNumeric(row.spend, 'spend'),
        ctr: null, // Not available in source
        cpc: row.spend && row.visits ? sanitizeNumeric(row.spend / row.visits, 'cpc') : null,
        impressions_mom: null, // Not available in source
        clicks_mom: null, // Not available in source 
        spend_mom: null, // Not available in source
        ctr_mom: null, // Not available in source
        cpc_mom: null // Not available in source
      }));
    ...
    console.log(`\n✅ Paid Ads sync complete: ${inserted} records`);
    return { success: true, records: inserted };
...
async function syncAllTables() {
  ...
  if (hasFailures) {
    console.log('\n⚠️  Some syncs failed. Please check the error messages above.');
    console.log('📝 Make sure all tables exist with the correct schema.');
    console.log('📖 See SYNC_TABLES_INSTRUCTIONS.md for SQL commands to create/update tables.');
  }
}
